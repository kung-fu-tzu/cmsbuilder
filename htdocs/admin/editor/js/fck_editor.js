...
if (typeof(URLParams['imagesFolder']) != "undefined")
{
   config.ImageBrowserURL = config.ImageBrowserURL + "?folder=" + URLParams['imagesFolder'] ;
   config.ImageUploadURL = config.ImageUploadURL + "?folder=" + URLParams['imagesFolder'] ;
}   
if (typeof(URLParams['attachFolder']) != "undefined")
{
   config.LinkBrowserURL = config.LinkBrowserURL + "?folder=" + URLParams['attachFolder'] ;
   config.LinkUploadURL = config.LinkUploadURL + "?folder=" + URLParams['attachFolder'] ;
}   
...
function onPaste()
{
	...
	else if (config.AutoDetectPasteFromWord && BrowserInfo.IsIE55OrMore)
	{
		...
		if ( re.test( sHTML ) )
		{
			if ( confirm('The text you want to paste seams to be copied from Word. Do you want to clean it before pasting?') )
			{
				cleanAndPaste( sHTML ) ;
				return false ;
			}
