...
package modControlPanel::Backup;
use strict qw(subs vars);
use utf8;

our @ISA = qw(modControlPanel::FileManager CMSBuilder::Module); #modAdmin::Simple
...
our $confname = 'config_v1.6.txt';

sub mod_load
{
	my $c = shift;
	
	push @modControlPanel::ControlPanel::ISA, $c;
}
...
sub fileman_cmenu
{
	...
	.
	($o->is_backup($o->fileman_localdir() . $path) && '
	elem_add(JMIHR());
...
sub is_backup
{
	...
	my $zip = eval { Archive::Zip->new($path) };
	
	return unless !$@ && $zip && $zip->memberNamed($confname);
}
...
sub cpanel_backup_restore
{
	...
	return $o->err_add('Неверный формат файла!') unless $o->is_backup($o->fileman_localdir().'/'.$path);
