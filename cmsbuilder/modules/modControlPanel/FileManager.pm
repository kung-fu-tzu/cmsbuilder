...
package modControlPanel::FileManager;
...
sub fileman_view
{
	...
	if($ftype eq 'dir')
	{
		...
		if($path)
		{
			...
			$up = '<a href="'.$o->admin_right_href().'&act=fileman_view&path='.$rpath.'"><img src="icons/dirup.png"></a>';
		}
		
		print '<fieldset class="mod_fileman"><legend>',$up,'&nbsp;',$path?$path:'/','</legend>';
...
sub fileman_icon
{
	...
	if($ftype eq 'dir'){ return $CMSBuilder::Config::http_userdocs.'/icons/directory.png'; }
