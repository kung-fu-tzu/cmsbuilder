...
sub admin_cmenu_for_self
{
	...
	my $code = $o->CMSBuilder::DBI::Object::OAdmin::admin_cmenu_for_self(@_);
	...
	if($o->access('c'))
	{
		...
		'
		elem_add(JMIHR());
		elem_add(JMIHref("Разрешения…","right.ehtml?url='.$o->myurl().'&act=access_chmod"));
		';
	}
...
sub access_add
{
	...
	$sth = $dbh->prepare('SELECT ID FROM `access` WHERE memb = ? AND url = ?');
	...
	$sth = $dbh->prepare('INSERT INTO `access` (url,memb,code) VALUES (?,?,?)');
	$sth->execute($o->myurl(),$m,$code || 0);
	
	$sth = $dbh->prepare('SELECT count(memb) FROM `access` WHERE url = ?');
	$sth->execute($o->myurl());
...
sub access_add_list
{
	...
	$str = $dbh->prepare('SELECT memb,code FROM `access` WHERE url = ?');
	$str->execute($o->myurl());
	...
	$count = 0;
	for $tg (modUsers::Group->sel_where(' 1 '))
	{
...
sub access_view
{
	...
	$sth = $dbh->prepare('SELECT memb,code FROM `access` WHERE url = ?');
	$sth->execute($o->myurl());
	...
	function DelMemb()
	{
		...
		var murl = memb.value;
		location.href = "?url=',$o->myurl(),'&act=access_chmod&chact=del&memb=" + murl;
	}
...
sub access_edit
{
	...
	$str = $dbh->prepare('UPDATE `access` SET code = ? WHERE memb = ? AND url = ?');
...
sub access_load
{
	...
	$str = $dbh->prepare('SELECT memb,code FROM `access` WHERE url = ? and memb in (?,?,?,?)');
