...
our @ISA = qw(modSite::Member modAdmin::Tree CMSBuilder::Module);

sub _cname {'Регистрация'}
sub _add_classes {qw(!*)}
sub _have_icon {1}
sub _pages_direction {0}
sub _aview{qw(group class)}
...
sub site_content
{
	...
	if($r->{'act'} eq 'register')
	{
		...
		if($r->{'email'})
		{
			...
			modUsers::API->login($tu->{'login'},$tu->{'pas'});
	...
	if($r->{'act'} eq 'login')
	{
		if(modUsers::API->login($r->{'login'},$r->{'password'}))
		{
			print '<script>location.href = "/"</script>';
		}
		else
		{
			print '<div class="message"><span class="head">Ошибка!</span> ',modUsers::API->last_error(),'</div>';
		}
	}
