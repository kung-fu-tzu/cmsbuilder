...
package modAdmin::Simple;
use strict qw(subs vars);
use utf8;

our @ISA = qw(CMSBuilder::DBI::Object::ONoBase CMSBuilder::DBI::Object modAdmin::RootElement);
...
sub simplem_menu
{
	...
	$$buff = {cmsb_varr(ref($o),'_simplem_menu')};
	return $$buff;
}
...
sub smodule_make_tree
{
	...
	if($obj->{'-obj'})
	{
		...
		$ret = {-name => $to->admin_name(), -id => $o->jsid.'_'.$to->jsid} if $to && $to->access('r');
	}
	elsif($obj->{'-sub'})
	{
		$ret = {-name => $o->name(), -id => $o->jsid.'_'.lc($obj->{'-sub'}), $obj->{'-sub'}->($o,$obj)};
	}
	else
	{
		...
		(
			...
			-icon	=> $obj->{'-icon'} || 'icons/default.png',
			-selid	=> 'id_'.$o->jsid.'_'.$obj->{'-func'}
		);
		
		$ret = {-name => $name, -id => $o->jsid.'_'.$obj->{'-func'}};
	}
...
sub admin_view_left
{
	...
	if($o->isa('modAdmin::Tree'))
	{
		$norooticon = 1;
		$tree->{'-id'} = $o->jsid.'_func';
	}
	else
	{
...
sub admin_path_html
{
	...
	my $ret = $o->CMSBuilder::DBI::Object::OAdmin::admin_path_html(@_);
	...
	return $ret.' / '.$o->admin_cname($rpcf->{$func}->{'-name'},$o->admin_right_href().'&act='.$func,'admin_right',$rpcf->{$func}->{'-icon'}||'icons/default.png');
}
...
sub admin_path_js
{
	...
	$o->CMSBuilder::DBI::Object::OAdmin::admin_path_js(@_);
	
	print 'parent.admin_left.CMS_SelectLO("id_'.$o->jsid.'_'.$r->{'act'}.'");';
	print 'parent.admin_left.CMS_ShowMe("'.$o->jsid.'_'.$r->{'act'}.'");';
}
