...
sub uldir { return $CMSBuilder::Config::path_userdocs.'/'.$user->jsid; }
sub uhdir { return $CMSBuilder::Config::http_userdocs.'/'.$user->jsid; }
...
sub list
{
	...
	opendir($ud,$uldir.$chdir) || warn "Cannot opendir($uldir$chdir): $!";
...
{
	my $fname = shift;
	return '<a onclick="return deleteConfirm(\''.$fname.'\')" href="?act=del&fname='.$fname.'&chdir='.$chdir.'"><img src="icons/del.png"></a>&nbsp;';
}
...
sub fileman_icon
{
	...
	if(-f $CMSBuilder::Config::path_userdocs.'/icons/'.$ext.'.png'){ return $CMSBuilder::Config::http_userdocs.'/icons/'.$ext.'.png'; }
	
	return $CMSBuilder::Config::http_userdocs.'/icons/default.png';
}
