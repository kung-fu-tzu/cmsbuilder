...
sub user_xinfo
{
	...
	elsif($name eq 'main_menu')
	{
		...
		if($group->{'cpanel'})
		{
			print '<a title="Обновить структуру" href="',modControlPanel->new(1)->admin_right_href(),'&act=cpanel_table_fix" target="admin_right"><img src="icons/table.gif" /></a><img src="img/nx.gif" />';
		}
		...
		if($group->{'cpanel'})
		{
			print modControlPanel->new(1)->admin_toolbar_icon();
			$cnt++;
		}
